...
SELECT DISTINCT
	...
	,membercost
	,monthlymaintenance
	,ROUND(membercost / monthlymaintenance * 100,1) AS membership_percentage
FROM
	Facilities
WHERE
...
*/
FROM
	(SELECT memid,facid,slots FROM Bookings WHERE DATE(starttime) = '2012-09-14') AS b
INNER JOIN
	Members AS m
	ON m.memid = b.memid
LEFT JOIN
